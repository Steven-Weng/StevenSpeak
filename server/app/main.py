"""FastAPI application entrypoint for the StevenSpeak orchestrator."""
...
from agents.realtime.config import RealtimeUserInputMessage, RealtimeSessionModelSettings
...
#     application = FastAPI(
#         title="StevenSpeak Agent Orchestrator",
#         description=(
...
class RealtimeWebSocketManager:
    def __init__(self):
        ...
        # Selected persona per session (joi | officer_k)
    ...
    async def connect(self, websocket: WebSocket, session_id: str):
        ...
        task = asyncio.create_task(self._process_events(session_id), name=f"stevenspeak-process-events-{session_id}")
    ...
    def _get_persona_voice_id(self, persona: str) -> str:
        ...
        return voice_mapping.get(persona.lower(), "en-US-AriaNeural")
    ...
    try:
        while True:
            ...
            if message["type"] == "audio":
                ...
                if persona not in {"joi", "officer_k"}:
                    await websocket.send_text(
...
async def root() -> dict[str, str]:
    ...
    return {"service": "stevenspeak-realtime", "status": "ok"}
